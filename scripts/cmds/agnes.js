...
async function askOpenAI(prompt, history = []) {
    const messages = [
        { role: 'system', content: 'You are Agnes, a smart, friendly, slightly witty AI assistant made by Broken_vzn. Be helpful, concise, and have personality. Use emojis naturally.' },
...
export default {
    config: {
        ...
        author: 'Broken_vzn',
        version: '2.0',
        shortDescription: 'Agnes AI — chat, generate images & videos',
        category: 'ai',
        coolDown: 3,
        role: 0,
        guide: {
    ...
    async onStart({ args, reply, sender, prefix, message, React }) {
        ...
        if (!args.length) {
            return reply([
                ...
                ``,
                `_Reply to any Agnes message to continue the conversation_`,
                `━━━━━━━━━━━━━━━━━━━━`,
            ].join('\n'));
        }
